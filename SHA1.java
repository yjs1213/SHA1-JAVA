...
public class SHA1 {
    ...
    public String spiltMsg(String str){ //메시지를 블록화합니다.
        ...
        for(int i = 0; i< str.length(); i++){ // 메시지를 16진수로 변환해서 binarymsg에 저장합니다.
            binarymsg.append(String.format("%02X", (int) str.charAt(i)));
        }
    ...
    public void test(String str, int num, int len){
        ...
        int[] block2 = {0x80000000, 0x00000000, 0x00000000, 0x00000000, 0x00000000, 0x00000000, 0x00000000, 0x00000000,
                0x00000000, 0x00000000, 0x00000000, 0x00000000, 0x00000000, 0x00000000, 0x00000000, 0x00000000};
        int loop = 0;
        StringBuilder BinedString = new StringBuilder();
        ...
        for(int i = 0; i<loop;i++){ //block에 정수형 변환해서 저장
            if (i==loop-1) block[i] = Integer.parseUnsignedInt(str.substring(i*8, str.length()),16);
            else block[i] = Integer.parseUnsignedInt(str.substring(i*8, i*8+8),16);
        }

        for(int k = 0; k < loop; k++){ // BinedString에 2진수 string으로 저장
            tmp = new StringBuilder(Integer.toBinaryString(block[k]));
            while(tmp.length()%8!=0){ // block이 정수형이기 때문에 앞에 0이 사라지므로 추가해줍니다.
                tmp.insert(0, "0");
            }
            BinedString.append(tmp);
        }

        if(BinedString.length()!=512){ //1을 무조건 패딩합니다. 단, 512비트의 경우 다음 블록에 1이 추가되므로 제외시킵니다.
            BinedString.append("1");
            while(BinedString.length()%8!=0) BinedString.append("0"); // 0도 패딩해줍니다.
        }
        switch(num){
            case 1: // 길이가 448비트일때 && 마지막 블록일때
                block1[15] = padlen(len); // 마지막 워드에 메시지 길이를 추가하고
                while(BinedString.length()%448!=0){
                    BinedString.append("00000000");
                }
            ...
            case 2: // 길이가 512비트일때 && 마지막 블록일때
                ...
                break;
            default: //일반적인 상황일때
                while(BinedString.length()% num !=0){
                    BinedString.append("00000000");
                }
    ...
    public String makeHash(){ // 계산된 해시를 합쳐서 String으로 변환해줍니다.
        ...
        StringBuilder tmp = new StringBuilder();
        for(int i = 0; i < 5; i++){
            tmp = new StringBuilder(Integer.toHexString(h[i]));
            while(tmp.length()!=8) tmp.insert(0, "0");
            value.append(tmp);
        }
